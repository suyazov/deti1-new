...
3. ~~Prerender не содержит изображения секций~~ — ✅ исправлено: добавлены hero-фото и фото в «Почему родители нас выбирают».
4. ~~Нет BreadcrumbList schema~~ — ✅ исправлено: добавлен `BreadcrumbList`.
5. **Слабая перелинковка** — в prerender только 7 ссылок, нет topic clusters (не делаем по запросу).
...
- ✅ Добавлена отдельная сущность `Person` для основателя с `sameAs`.
- ✅ Обновлен `og:image` на hero-фото WebP 1050w.
- ✅ Добавлены изображения в prerender.

**Оставшиеся P1:**
- Добавить раздел «Отзывы партнёров» с Review schema.
- Добавить editorial policy / corrections page.
- Расширить prerender внутренними ссылками (если потребуется позже).
