...
export function StepsSlider() {
  ...
  return (
    <section className="section-dark py-12 md:py-16 xl:py-10 relative overflow-hidden">
      <div className="glow-orb w-[500px] h-[500px] bg-[#dc2626]/10 -left-60 top-1/2" />
          ...
          {steps.map((s, i) => (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 sm:gap-2 p-2 sm:p-3 rounded-xl sm:rounded-2xl transition-all duration-300 ${
                selected === i
                  ? 'bg-[#00796b]/15 border border-[#00796b]/30'
            ...
            >
              <div
                className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center transition-colors ${
                  selected === i ? 'bg-[#00796b] text-[#0a1f1f]' : 'bg-black/5 text-[#636e72]'
                }`}
              >
                <s.icon className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <span className={`text-[10px] sm:text-xs font-medium hidden md:block ${selected === i ? 'text-[#00796b]' : 'text-[#636e72]'}`}>
                {s.title}
              </span>
            </button>
          ))}
                ...
                <div className="glass-card rounded-2xl sm:rounded-[28px] lg:rounded-[32px] p-4 sm:p-5 md:p-8 xl:p-5 grid md:grid-cols-2 gap-4 sm:gap-6 md:gap-10 xl:gap-6 items-center">
                  <div>
                    <div className="inline-flex items-center gap-2 glass-card rounded-full px-3 sm:px-4 py-1.5 mb-3 sm:mb-4">
                      <span className="text-[10px] sm:text-xs font-bold text-[#00796b]">{s.num}</span>
                      <span className="text-[10px] sm:text-xs text-[#636e72]">{s.desc}</span>
                    </div>
                    <h3 className="text-xl sm:text-2xl md:text-3xl xl:text-2xl font-bold text-[#2D3436] mb-3 sm:mb-4">{s.title}</h3>
                    <p className="text-[#636e72] leading-relaxed text-sm sm:text-base md:text-lg xl:text-sm">{s.detail}</p>
                  </div>
                  <div className="relative">
                    <div className="absolute inset-0 bg-gradient-to-br from-[#00796b]/20 to-transparent rounded-xl sm:rounded-2xl blur-2xl" />
